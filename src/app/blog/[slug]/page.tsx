...
// Prefetching must happen after connection(): the Docker build has no
// database, so the view-count query can only resolve at request time.
...
const ViewCounterSection = async ({ slug }: { slug: string }) => {
   await connection();
   prefetch(trpc.blog.get.queryOptions({ slug }));
   return (
      <HydrateClient>
   ...
   return (
      <section>
         ...
         <div className="mt-4 mb-8 flex items-center gap-3 text-sm text-muted-foreground">
            ...
            <Suspense
               fallback={
                  <span className="font-mono text-sm tracking-tighter tabular-nums">
                     <Skeleton className="inline-block h-3.5 w-14 rounded align-middle" />
                  </span>
               }
            >
               <ViewCounterSection slug={post.slug} />
            </Suspense>
         </div>
         <Mdx code={post.body} />
         <div className="my-4" />
