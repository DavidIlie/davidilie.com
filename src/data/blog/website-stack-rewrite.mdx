...
```ts title="Typescript Type for Gear Page"
...
Like I mentioned before, I used to use MongoDB, but now I switched to an <Definition term="SQL" definition="Structured Query Language"/> called `PostgreSQL`, I just like it much more because of how well it integrates with `Prisma` and the idea of using schemas/tables in Postgres instead of documents in Mongo.
...
Otherwise I've mostly spent upgrading the website to match my current coding standards, aka using better principles for coding (both for react and/or typescript in general), etc.

As for deployment, the plan in the future is to keep the website running on my Kubernetes Cluster, but since I am moving to study in the United Kingdom, and moving with my family to Romania, my servers hosting my cluster are currently offline, which is a shame. But until then, the plan is to host it all on the cloud.
