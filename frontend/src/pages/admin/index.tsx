...
const Admin = ({ statistics }: AdminProps): JSX.Element => {
    ...
    const SelectionTabClassName = (selected: boolean) =>
        selected
            ? "bg-blue-600 dark:bg-blue-800 shadow text-white"
            : "text-gray-800 dark:text-blue-100 hover:bg-gray-300 dark:hover:bg-gray-700 hover:text-black dark:hover:text-white";

    const TabClassName = (selected: boolean) =>
        `${DefaultTabClassName} ${SelectionTabClassName(selected)}`;

    return (
        <AdminPage className="h-screen pt-28 max-w-7xl mx-auto pl-8 px-4 sm:px-6 sm:pl-8 lg:px-8 lg:pl-8">
            <Fade direction="up">
                <div>
                    <h1 className="xl:text-5xl text-4xl font-semibold mb-5">
                        Welcome back{" "}
                        <span className="header-gradient">David!</span>
                    </h1>
                    <Tab.Group>
                        <Tab.List className="flex p-1 space-x-1 bg-gray-100 dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-900">
                            <Tab
                                className={({ selected }) =>
