// all the colors that need to stay when the app is built in prod :))

export default function tailwindbad() {
    return (
        <>
            <h1 className="text-gray-100">fuck you tailwind</h1>
            <h1 className="text-gray-200">fuck you tailwind</h1>
            <h1 className="text-gray-300">fuck you tailwind</h1>
            <h1 className="text-gray-400">fuck you tailwind</h1>
            <h1 className="text-gray-500">fuck you tailwind</h1>
            <h1 className="text-gray-600">fuck you tailwind</h1>
            <h1 className="text-gray-700">fuck you tailwind</h1>
            <h1 className="text-indigo-100">fuck you tailwind</h1>
            <h1 className="text-indigo-200">fuck you tailwind</h1>
            <h1 className="text-indigo-300">fuck you tailwind</h1>
            <h1 className="text-indigo-400">fuck you tailwind</h1>
            <h1 className="text-indigo-500">fuck you tailwind</h1>
            <h1 className="text-blue-100">fuck you tailwind</h1>
            <h1 className="text-blue-200">fuck you tailwind</h1>
            <h1 className="text-blue-300">fuck you tailwind</h1>
            <h1 className="text-blue-400">fuck you tailwind</h1>
            <h1 className="text-blue-500">fuck you tailwind</h1>
            <h1 className="text-green-100">fuck you tailwind</h1>
            <h1 className="text-green-200">fuck you tailwind</h1>
            <h1 className="text-green-300">fuck you tailwind</h1>
            <h1 className="text-green-400">fuck you tailwind</h1>
            <h1 className="text-green-500">fuck you tailwind</h1>
            <h1 className="text-red-100">fuck you tailwind</h1>
            <h1 className="text-red-200">fuck you tailwind</h1>
            <h1 className="text-red-300">fuck you tailwind</h1>
            <h1 className="text-red-400">fuck you tailwind</h1>
            <h1 className="text-red-500">fuck you tailwind</h1>
            <h1 className="text-yellow-100">fuck you tailwind</h1>
            <h1 className="text-yellow-200">fuck you tailwind</h1>
            <h1 className="text-yellow-300">fuck you tailwind</h1>
            <h1 className="text-yellow-400">fuck you tailwind</h1>
            <h1 className="text-yellow-500">fuck you tailwind</h1>
            <h1 className="bg-gray-100">fuck you tailwind</h1>
            <h1 className="bg-gray-200">fuck you tailwind</h1>
            <h1 className="bg-gray-300">fuck you tailwind</h1>
            <h1 className="bg-gray-400">fuck you tailwind</h1>
            <h1 className="bg-gray-500">fuck you tailwind</h1>
            <h1 className="bg-gray-600">fuck you tailwind</h1>
            <h1 className="bg-gray-700">fuck you tailwind</h1>
            <h1 className="bg-indigo-100">fuck you tailwind</h1>
            <h1 className="bg-indigo-200">fuck you tailwind</h1>
            <h1 className="bg-indigo-300">fuck you tailwind</h1>
            <h1 className="bg-indigo-400">fuck you tailwind</h1>
            <h1 className="bg-indigo-500">fuck you tailwind</h1>
            <h1 className="bg-indigo-600">fuck you tailwind</h1>
            <h1 className="bg-indigo-700">fuck you tailwind</h1>
            <h1 className="bg-indigo-800">fuck you tailwind</h1>
            <h1 className="bg-blue-100">fuck you tailwind</h1>
            <h1 className="bg-blue-200">fuck you tailwind</h1>
            <h1 className="bg-blue-300">fuck you tailwind</h1>
            <h1 className="bg-blue-400">fuck you tailwind</h1>
            <h1 className="bg-blue-500">fuck you tailwind</h1>
            <h1 className="bg-blue-600">fuck you tailwind</h1>
            <h1 className="bg-blue-700">fuck you tailwind</h1>
            <h1 className="bg-blue-800">fuck you tailwind</h1>
            <h1 className="bg-green-100">fuck you tailwind</h1>
            <h1 className="bg-green-200">fuck you tailwind</h1>
            <h1 className="bg-green-300">fuck you tailwind</h1>
            <h1 className="bg-green-400">fuck you tailwind</h1>
            <h1 className="bg-green-500">fuck you tailwind</h1>
            <h1 className="bg-green-600">fuck you tailwind</h1>
            <h1 className="bg-green-700">fuck you tailwind</h1>
            <h1 className="bg-green-800">fuck you tailwind</h1>
            <h1 className="bg-red-100">fuck you tailwind</h1>
            <h1 className="bg-red-200">fuck you tailwind</h1>
            <h1 className="bg-red-300">fuck you tailwind</h1>
            <h1 className="bg-red-400">fuck you tailwind</h1>
            <h1 className="bg-red-500">fuck you tailwind</h1>
            <h1 className="bg-red-600">fuck you tailwind</h1>
            <h1 className="bg-red-700">fuck you tailwind</h1>
            <h1 className="bg-yellow-100">fuck you tailwind</h1>
            <h1 className="bg-yellow-200">fuck you tailwind</h1>
            <h1 className="bg-yellow-300">fuck you tailwind</h1>
            <h1 className="bg-yellow-400">fuck you tailwind</h1>
            <h1 className="bg-yellow-500">fuck you tailwind</h1>
            <h1 className="bg-yellow-600">fuck you tailwind</h1>
            <h1 className="bg-yellow-700">fuck you tailwind</h1>
            <h1 className="bg-yellow-800">fuck you tailwind</h1>
        </>
    );
}
