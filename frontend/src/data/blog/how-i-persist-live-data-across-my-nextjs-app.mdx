...
summary: "While working on one of my projects I came across a problem with fetching data such as settings from my backend. The problem was that I didn't want to do the request on every request, just when I wanted it to be fetched."
...
While working on one of my projects ([sharex-media-server](https://github.com/davidilie/sharex-media-server)) I came across a problem with fetching data such as settings from my backend. The problem was that I didn't want to do the request on **every** request, just in the moments where I wanted it to be fetched (such as updating the profile or force-reload).
...
// every component which would need this data
```
...
```tsx:web/src/pages/control-panel.tsx
...
```ts:web/src/global-stores/useSettingsStore.ts
...
useEffect(() => {
    if (performance.navigation.type != 1) {
        if (settings.name && user.name)
            return setFinishedDataCheck(true);
    }
    const getData = async () => {
...
```ts:web/src/hooks/useSettings.ts
...
```tsx:web/src/modules/user/profile/profile-information.tsx
const { user, updateUser } = useUserStore();
```
...
<span className="flex justify-center pb-3">
    <a href="https://github.com/DavidIlie/sharex-media-server" target="_blank">
        <img src="https://github-readme-stats.vercel.app/api/pin/?username=DavidIlie&repo=sharex-media-server&title_color=fff&icon_color=79ff97&text_color=9f9f9f&bg_color=151515" />
    </a>
</span>
